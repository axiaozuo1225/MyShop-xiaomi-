...
public class EmailUtils {
    public static void sendEmail(User user) {
        //发送方
        String myAccount = "axiaozuo1225@163.com";
        //授权码
        String myPass = "420984.he";
        //发件人 邮箱的 SMTP 服务器地址
    ...
    private static MimeMessage createMsg(Session session, String myAccount, User user) {
        ...
        try {
            ...
            message.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(user.getEmail(), "尊敬的用户" + 666, "utf-8"));
            //message.setRecipient(MimeMessage.RecipientType.CC, new InternetAddress(user.getEmail(), user.getUsername(), "utf-8"));
            //3.3生成邮件主题
            message.setSubject("小米商城账号激活邮件", "utf-8");

            String ip = Inet4Address.getLocalHost().getHostAddress();
            String url = "http://" + ip + ":8080/user?method=active&c=" + Base64Utils.encode(String.valueOf(user.getCode()));
            //String url = "http://"+ip+":8080/user?method=activeUser&c="+Base64Utils.encode(String.valueOf(user.getCode()));//发送激活链接给接收方的用户点击跳转到后台处理激活逻辑
            ...
            message.setContent(user.getUsername() + ",你好<br>欢迎注册小米商城! 请点击链接进行激活:<a href='" + url + "'>" + "点击激活" + "</a>", "text/html;charset=utf-8");
            //设置邮件的发送时间 是立即发送
